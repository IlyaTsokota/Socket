...
contact_info_t *get_contact_info(char *contact_id)
{
        ...
        char *arr[] = {contact_id, data.user_id, NULL};
        ...
        free(values_name);
        json_object_object_get_ex(tmp_values, "is_my_contact", &values_name);
        info->is_my_contact = json_object_get_boolean(values_name);
        free(values_name);
        free(tmp_values);
        free(values_obj);
        free(jobj);
