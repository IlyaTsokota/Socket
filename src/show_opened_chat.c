...
void show_opened_chat(GtkWidget *main_grid, char *chat_id)
{
    ...
    g_signal_connect(G_OBJECT(msg_entry->text_view), "key-press-event", G_CALLBACK(send_msg_on_press_enter), msg_entry->text_view);
...
{
    if (event->state & GDK_SHIFT_MASK && event->keyval == 65293)
    {
        return FALSE;
    }
    else if ((event->type == GDK_KEY_PRESS && event->keyval == 65293) || (event->type == GDK_CONTROL_MASK && event->keyval == 65293))
    {
        send_message(NULL, NULL, text_view);
    }
