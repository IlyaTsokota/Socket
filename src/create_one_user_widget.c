...
void create_one_user_widget(int i, user_curr_chat_t *contact)
{
    ...
    if (strcmp(data.user_id, contact->u_id) != 0)
    {
    ...
    if (!main_form.is_admin_by_chat || strcmp(data.user_id, contact->u_id) == 0)
    {
        gtk_widget_destroy(users_in_chat.users[i]->delete_event_box);
        users_in_chat.users[i]->delete_event_box = NULL;
    }

    users_in_chat.users[i]->remove_img = gtk_image_new_from_file("share/resources/img/delete.png");
    if (!main_form.is_admin_by_chat || strcmp(data.user_id, contact->u_id) == 0)
    {
        gtk_widget_destroy(users_in_chat.users[i]->remove_img);
        users_in_chat.users[i]->remove_img = NULL;
    }
    ...
    if (main_form.is_admin_by_chat || strcmp(data.user_login, contact->u_id) != 0)
    {
