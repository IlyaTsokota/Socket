    ...
    if (chats != NULL)
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
           ...
           char *time = strdup(&message->messages[last_item]->ms_datetime[11]);
            ...
            char last_msg[30];
            strncpy(last_msg, message->messages[last_item]->ms_text, 25);
            strcpy(&last_msg[25], "...\0");
            ...
            strcpy(&message->messages[last_item]->u_name[strlen(message->messages[last_item]->u_name)], ":\0");
            chats_f.chat_items[i]->login_last_message = gtk_label_new(message->messages[last_item]->u_name);
