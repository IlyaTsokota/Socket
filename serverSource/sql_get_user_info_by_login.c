...
char *get_user_info_by_login(MYSQL *con, char *login, int sock)
{
    const char *tmp_str, *coma_str = ",";
    char *str, *bdrequest, *tmp_str1;
    const char *request_parts[] = {"select u_id, concat(u_name, ' ', u_surname), u_avatar, u_status from user where u_login = \"", login, "\" limit 1;", NULL};
