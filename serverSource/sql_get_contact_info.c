...
char *get_contact_info(MYSQL *con, char *user_id, int sock)
{
    ...
    const char *request_parts[] = {"SELECT u_login, concat(u.u_name, ' ', u.u_surname), u_avatar, u_status FROM user where u_id = \"", user_id, "\";", NULL};
    ...
    while ((row = mysql_fetch_row(result)))
    {
        ...
        contact->u_status = strdup(row[2]);
        contact->u_avatar = strdup(row[3]);
